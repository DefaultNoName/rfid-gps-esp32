...
#define RST_PIN 0 // D3 - GPIO0 (RST pin for RFID) or
...
SoftwareSerial serial_gps(GPS_RX_PIN, -1);
    ...
    if (module_gps.encode(serial_gps.read()))
    {
      ...
      { // Update time if valid
        ...
        if (module_gps.location.isValid())
        {
          ...
          Serial.println(current_gps_lat);
          Serial.println(current_gps_lng);
        }
...
void handle_rfid_reading()
{
  ...
  if ((millis() - last_valid_read) < 30000) // Ignore for 30 seconds
  {
    cleanup_rfid();
    return;
  }

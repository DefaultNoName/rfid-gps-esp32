// Modified ESP32 Sensor Code - Fixed for JDY-16 Communication
// Key changes: Using FFE0 service and FFE1 characteristic UUIDs
...
// BLE Configuration - FIXED for JDY-16 actual UUIDs
#define BLE_DEVICE_NAME "Tracker-ESP32#1"
#define BLE_BRIDGE_NAME "ESP-BLE" // Name set in bridge ESP
// Updated to use JDY-16 actual service and characteristic UUIDs
#define BLE_SERVICE_UUID "0000FFE0-0000-1000-8000-00805F9B34FB"  // JDY-16 service
#define BLE_CHAR_UUID_WRITE "0000FFE1-0000-1000-8000-00805F9B34FB"  // JDY-16 characteristic
...
// IMPROVED BLE transmission function - optimized for JDY-16
void sendJsonViaBLE(const String &jsonData)
{
    ...
    // JDY-16 typically handles up to 20 bytes per packet reliably
    const size_t maxPacketSize = 20;
    size_t dataLength = jsonData.length();

    Serial.printf("Sending JSON data (%d bytes) via BLE to JDY-16:\n", dataLength);
    Serial.println(jsonData);

    // Send data in chunks with proper timing for JDY-16
    for (size_t i = 0; i < dataLength; i += maxPacketSize)
    {
        ...
        try
        {
            // Write to FFE1 characteristic - this goes directly to JDY-16 UART output
            pRemoteCharacteristic->writeValue(chunk.c_str(), chunk.length());
            Serial.printf("Sent chunk %d: %s\n", i / maxPacketSize + 1, chunk.c_str());
            
            // Longer delay for JDY-16 to process and forward via UART
            delay(100); // Increased delay for JDY-16 processing
        }
    ...
    // Send end marker to indicate complete JSON - important for ESP8266 parsing
    delay(200);
    pRemoteCharacteristic->writeValue("\n", 1);
    Serial.println("JSON transmission complete with end marker");
}

// State machine functions (init and scanning remain the same)
...
{
    Serial.println("Initializing BLE and scanning for JDY-16 bridge...");
    ...
    // Start scanning
    Serial.printf("Scanning for JDY-16 bridge device: %s\n", BLE_BRIDGE_NAME);
...
// MAJOR CHANGES IN CONNECTION HANDLING
void handle_ble_connecting()
{
    // Check if we found a device to connect to
    if (doConnect)
    {
        Serial.printf("Attempting connection to JDY-16 at: %s\n", deviceAddress.c_str());
        ...
        try
        {
            ...
            Serial.println("Connected to JDY-16 bridge");

            // CRITICAL FIX: Get the JDY-16 service (FFE0)
            BLERemoteService *pRemoteService = pClient->getService(BLE_SERVICE_UUID);
            if (pRemoteService == nullptr)
            {
                Serial.println("Failed to find JDY-16 service FFE0");
                pClient->disconnect();
                current_system_state = STATE_BLE_SCANNING;
                return;
            }
            Serial.println("Found JDY-16 service FFE0");

            // CRITICAL FIX: Get the JDY-16 characteristic (FFE1)
            pRemoteCharacteristic = pRemoteService->getCharacteristic(BLE_CHAR_UUID_WRITE);
            if (pRemoteCharacteristic == nullptr)
            {
                Serial.println("Failed to find JDY-16 characteristic FFE1");
                pClient->disconnect();
                current_system_state = STATE_BLE_SCANNING;
                return;
            }
            Serial.println("Found JDY-16 characteristic FFE1");

            // Check if we can write to the characteristic
            if (!pRemoteCharacteristic->canWrite())
            {
                Serial.println("Warning: FFE1 characteristic may not support write operations");
                // Continue anyway, some JDY-16 modules still work
            }

            // Optional: Check if the characteristic supports notifications (for bidirectional communication)
            if (pRemoteCharacteristic->canNotify())
            {
                Serial.println("JDY-16 supports notifications - setting up callback");
                pRemoteCharacteristic->registerForNotify([](BLERemoteCharacteristic *pBLERemoteCharacteristic,
                                                            uint8_t *pData, size_t length, bool isNotify)
                                                         { 
                                                             Serial.printf("Received from JDY-16: %.*s\n", length, pData); 
                                                         });
            }
            ...
            Serial.println("JDY-16 connection setup complete!");
        }
        catch (const std::exception &e)
        {
            Serial.printf("JDY-16 connection failed: %s\n", e.what());
            current_system_state = STATE_BLE_SCANNING;
            doConnect = false;
        }
    }
...
void handle_hardware_init()
{
    ...
    Serial.println("System ready - JDY-16 communication active");
}

// GPS helper functions (unchanged)
...
// RFID helper functions (unchanged)
...
// Main JSON transmission function (unchanged)
...
void handle_running_state()
{
    ...
    if (!deviceConnected)
    {
        Serial.println("JDY-16 disconnected, returning to scanning");
        current_system_state = STATE_BLE_SCANNING;
        return;
    }

    // Handle RFID reading
    if (components.rfid_ok && current_time - last_rfid_read > 5000)
    {
        handle_rfid_reading();
    ...
    // Handle GPS reading
    if (components.gps_ok && current_time - last_gps_update > 5000)
    {
        handle_gps_reading();
        last_gps_update = current_time;
    }

    // Handle JSON transmission via BLE to JDY-16
...
void setup()
{
    ...
    Serial.println("=== Vehicle Tracking System (JDY-16 BLE) ===");
